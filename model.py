...
def train_model(citywide_df, cuny_df):
    ...
    cuny_df = cuny_df.dropna(
        subset=["Street level air temperature", "Street level relative humidity"]
    )
    ...
    adj_r2_offset = adjusted_r2(
        r2_offset, X_test_offset.shape[0], X_test_offset.shape[1]
    )
    ...
    platform_daily = (
        merged_df.groupby(["Date", "gtfs_stop_id"])["Platform level air temperature"]
        .mean()
        .reset_index()
    )
    platform_daily.rename(
        columns={"Platform level air temperature": "Prev_Platform_Temp"}, inplace=True
    )
    ...
    X_train_pf, X_test_pf, y_train_pf, y_test_pf = train_test_split(
        platform_features, platform_target, test_size=0.2, random_state=42
    )
    ...
    adj_r2_pf_off = adjusted_r2(
        r2_pf_off, X_test_pf_off.shape[0], X_test_pf_off.shape[1]
    )
    ...
    if st.button("Predict Platform and Street Temperature"):
        try:
            ...
            humidity_input = humidity_input.reindex(
                columns=humidity_feature_cols, fill_value=0
            )
            ...
            offset_input_df = offset_input_df.reindex(
                columns=offset_model.feature_names_in_, fill_value=0
            )
            ...
            platform_offset_input_df = platform_offset_input_df.reindex(
                columns=platform_offset_model.feature_names_in_, fill_value=0
            )
            platform_offset_pred = platform_offset_model.predict(
                platform_offset_input_df
            )[0]
            platform_temp_pred_offset = street_level_temp_pred + platform_offset_pred
            st.info(
                f"Predicted Platform-Level Temperature (Offset Model): {platform_temp_pred_offset:.2f} °F (Predicted Offset: {platform_offset_pred:+.2f} °F)"
            )
            ...
            true_pf_temp = (
                y_test_pf_off + X_test_pf_off["Street level air temperature"].values
            )
            pred_pf_temp = (
                y_pred_pf_off + X_test_pf_off["Street level air temperature"].values
            )
            ...
            ax2.plot(
                [true_pf_temp.min(), true_pf_temp.max()],
                [true_pf_temp.min(), true_pf_temp.max()],
                "r--",
            )
