...
def load_data():
    ...
    cuny_df["Platform level relative humidity"] = pd.to_numeric(
        cuny_df["Platform level relative humidity"], errors="coerce"
    )
    cuny_df["Street level air temperature"] = pd.to_numeric(
        cuny_df["Street level air temperature"], errors="coerce"
    )
    cuny_df[
        "Street level air temperature - Sunny conditions (complete only if there is no shady spot near the subway entrance)"
    ] = pd.to_numeric(
        cuny_df[
            "Street level air temperature - Sunny conditions (complete only if there is no shady spot near the subway entrance)"
        ],
    ...
    cuny_df["Platform level air temperature"] = pd.to_numeric(
        cuny_df["Platform level air temperature"], errors="coerce"
    )
    cuny_df["Street level relative humidity"] = pd.to_numeric(
        cuny_df["Street level relative humidity"], errors="coerce"
    )

    cuny_df["Street level air temperature"] = cuny_df[
...
def train_model(citywide_df, cuny_df):
    ...
    merged_df["Hour"] = pd.to_datetime(
        merged_df["Time for street level data collection"],
        format="%I:%M:%S %p",
        errors="coerce",
    ).dt.hour
    ...
    X_train_h, X_test_h, y_train_h, y_test_h = train_test_split(
        humidity_features, humidity_target, test_size=0.2, random_state=42
    )
    ...
    r2_h, adj_r2_h, mae_h, rmse_h = compute_metrics(
        y_test_h, y_pred_h, X_test_h.shape[0], X_test_h.shape[1]
    )

    # Street-Level Temperature Offset Model
    merged_df["Offset"] = (
        merged_df["Street level air temperature"] - merged_df["High Temp (°F)"]
    )
    ...
    X_train_offset, X_test_offset, y_train_offset, y_test_offset = train_test_split(
        offset_features, offset_target, test_size=0.2, random_state=42
    )
    offset_model = RandomForestRegressor(
    ...
    r2_offset, adj_r2_offset, mae_offset, rmse_offset = compute_metrics(
        y_test_offset, y_pred_offset, X_test_offset.shape[0], X_test_offset.shape[1]
    )
    ...
    prev_platform = platform_daily[
        ["gtfs_stop_id", "Prev_Date", "Prev_Platform_Temp", "Prev_Platform_Humidity"]
    ]

    platform_df = pd.merge(
        merged_df, prev_weather, left_on="Date", right_on="Prev_Date", how="left"
    )
    platform_df = pd.merge(
        platform_df,
        prev_platform,
        left_on=["Date", "gtfs_stop_id"],
        right_on=["Prev_Date", "gtfs_stop_id"],
        how="left",
        suffixes=("", "_prev_platform"),
    )
    platform_df = platform_df.dropna(
        subset=["Platform level air temperature", "Platform level relative humidity"]
    )

    # Platform Temperature Offset Model
    platform_df["Platform_Offset"] = (
        platform_df["Platform level air temperature"]
        - platform_df["Street level air temperature"]
    )
    ...
    r2_pf_off, adj_r2_pf_off, mae_pf_off, rmse_pf_off = compute_metrics(
        y_test_pf_off, y_pred_pf_off, X_test_pf_off.shape[0], X_test_pf_off.shape[1]
    )
    ...
    X_train_ph, X_test_ph, y_train_ph, y_test_ph = train_test_split(
        platform_humidity_features,
        platform_humidity_target,
        test_size=0.2,
        random_state=42,
    )
    ...
    r2_ph, adj_r2_ph, mae_ph, rmse_ph = compute_metrics(
        y_test_ph, y_pred_ph, X_test_ph.shape[0], X_test_ph.shape[1]
    )
...
try:
    ...
    station_name = st.selectbox(
        "Select Station", sorted(cuny_df["Station name"].dropna().unique())
    )
    ...
    if st.button("Predict All"):
        ...
        platform_offset_pred = platform_offset_model.predict(platform_offset_input_df)[
            0
        ]
        ...
        platform_rh_pred = platform_humidity_model.predict(platform_humidity_input_df)[
            0
        ]

        # Prediction
        st.success(
            f"Predicted Street-Level Relative Humidity: {predicted_humidity:.1f}%"
        )
        ...
        st.warning(
            f"Predicted Platform-Level Relative Humidity: {platform_rh_pred:.1f}%"
        )
        ...
        display_metrics(
            "Street Temp", r2_offset, adj_r2_offset, mae_offset, rmse_offset
        )
        display_metrics(
            "Platform Temp", r2_pf_off, adj_r2_pf_off, mae_pf_off, rmse_pf_off
        )
